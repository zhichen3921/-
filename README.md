...
4. 在投递台“浏览器扩展”页面生成扩展配对令牌，再在扩展设置中粘贴并保存。
...
运行后生成的岗位状态和沟通话术位于 `data/`，这些内容已被 `.gitignore` 排除。请使用应用内的“导出数据”功能备份，不要把真实求职数据提交到公共仓库。
...
- 当前版本不包含每日自动更新；岗位需要通过内置示例、手动添加或浏览器扩展主动采集。
